...
angular.module('JWTDemoApp', [ 'ui.router', 'ui.bootstrap' ])
...
.run(function(AuthService, $rootScope, $state) {
	...
	$rootScope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams) {
		...
		if (!AuthService.user) {
			...
			// if condition.
			if (toState.name != 'login' && toState.name != 'begin' && toState.name != 'secmer-vestido'
				&& toState.name != 'secmer-cigana' && toState.name != 'secmer-camisete'
				&& toState.name != 'begin' && toState.name != 'register') {
